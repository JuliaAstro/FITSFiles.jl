...
function FieldFormat(
        ::Type{Table}, format::DataFormat, reskeys::Dict{S, V}, data::Missing;
        record::Bool = false, kwds...
...
function FieldFormat(
        ::Type{Table}, format::DataFormat, reskeys::Dict{S, V}, data::U;
        columns = nothing, formats = nothing, kwds...
    ) where
    {U <: Union{Tuple, NamedTuple}, S <: AbstractString, V <: ValueType}
...
function create_data(
        ::Type{Table}, format::DataFormat, fields::Vector{TableField}; kwds...
    )
    #  Create simple N-dimensional array of zeros of type BITPIX
    if length(format.shape) == 2
        return [repeat(" ", format.shape[1]) for j in 1:format.shape[2]]
    else
        return missing
    end
end
...
function Base.read(
        io::IO, field::TableField, format::DataFormat, begpos::Integer;
        scale::Bool = true
    )
...
function Base.read(
        row::AbstractString, field::TableField;
        scale::Bool = true, invalid::Bool = true
    )
