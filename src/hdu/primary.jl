...
function Base.read(
        io::IO, ::Type{Primary}, format::DataFormat, fields::ImageField;
        scale::Bool = true, kwds...
    )
...
function Base.write(
        io::IO, ::Type{Primary}, data::Missing, format::DataFormat,
        fields::ImageField; kwds...
    )
end

function Base.write(
        io::IO, ::Type{Primary}, data::AbstractArray,
...
function verify!(
        ::Type{Primary}, cards::Cards, format::DataFormat,
        mankeys::D
...
function FieldFormat(
        ::Type{Primary}, format::DataFormat, reskeys::Dict{S, V},
        data::Missing
    ...
    return ImageField(format.type, zero_, scale, miss, dmin, dmax)
end

function DataFormat(
        ::Type{Primary}, data::AbstractArray,
        mankeys::Dict{S, V}
    ) where {S <: AbstractString, V <: ValueType}
...
function create_cards!(
        ::Type{Primary}, format::DataFormat, fields::ImageField, cards::Cards;
        kwds...
    )
...
function create_data(::Type{Primary}, format::DataFormat, ::ImageField; kwds...)
...
function Base.read(io::IO, format::DataFormat, fields::ImageField)
    ...
    data = reshape(
        ntoh.(
            reinterpret(format.type, read(io, M * N))
        ), shape
    )
...
function apply_units(data::AbstractArray, fields::ImageField)
    return !isnothing(fields.unit) ? data * fields.unit : data
end

function remove_units(data::AbstractArray)
    return typeof(data) <: Quantity ? data.val : data
end
