...
const BITS2TYPE = Dict(
    8 => UInt8, 16 => Int16, 32 => Int32, 64 => Int64,
    -32 => Float32, -64 => Float64
)
const TYPE2BITS = Dict(
    UInt8 => 8, Int16 => 16, Int32 => 32, Int64 => 64,
    Float32 => -32, Float64 => -64
)

const MISSTYPE = (UInt8, Int16, Int32, Int64)
const TEMPTYPE = Dict(
    -2^7 => Int16, 2^15 => UInt32, UInt32(2)^31 => UInt64,
    UInt64(2)^63 => UInt128
)
const OUTTYPE = Dict(
...
function HDU(
        data::D = missing, cards::C = missing; record::B = false,
        scale::B = true, append::B = false, fixed::B = true, slash::I = 32, lpad::I = 1,
        rpad::I = 1, upad::I = 1, truncate::B = true
    ) where
    {
    ...
    return HDU(type, data, kards; kwds...)
end
...
function HDU(type::Type, data::AbstractArray, cards::C = missing; kwds...) where
    ...
    if eltype(data) <: Tuple
        data = [
            (;
                    [
                        Symbol(fields[j].name) => data[k][j]
                        for j in 1:format.param
                    ]..., data = data[k][end],
...
function HDU(type::Type, data::U, cards::C = missing; kwds...) where
    ...
    if typeof(data) <: Tuple
        data = (;
            [Symbol(fields[j].name) => data[j] for j in 1:format.param]...,
            data = data[end],
        )
    end
...
function info(hdu::HDU, n::Integer = 0)
    ...
    siz = join(
        [
            string(cards["NAXIS$j"])
                for j in (typeofhdu(hdu) == Random ? 2 : 1):N
        ], " × "
    )
...
function Base.getproperty(hdu::HDU, name::Symbol)
    if name === :data && getfield(hdu, :data) isa LazyArray
        ...
        seek(io, desc.begpos)
        field = read(
            io, typeofhdu(hdu), desc.format, desc.fields; desc.keywds...
        )
        close(io)
    else
...
function datasize(mankeys::Dict, start::Integer = 1)
    ...
    return N > 0 ? Tuple(mankeys["NAXIS$j"] for j in start:N) : ()
end
...
function datalength(mankeys::Dict, start::Integer = 1)
    ...
    return N > 0 ? G * (P + prod(datasize(mankeys, start))) : 0
end
...
function datasize(cards::Cards, start::Integer = 1)
    #  Determine the shape of the array
    N = datadims(cards)
    return N > 0 ? Tuple(cards["NAXIS$j"] for j in start:N) : ()
end
...
function databytes(cards::Cards, start::Integer = 1)
    ...
    return N > 0 ? sizeof(datatype(cards)) * G * (P + prod(datasize(cards, start))) : 0
end

function datalength(cards::Cards, start::Integer = 1)
    #  Calculate the number of elements in the array
    N, G, P = cards["NAXIS"], get(cards, "GCOUNT", 1), get(cards, "PCOUNT", 0)
    return N > 0 ? G * (P + prod(datasize(cards, start))) : 0
end
...
function padblock(io::IO, format::DataFormat, pad::UInt8 = 0x00)
    ...
    return Base.write(io, fill(pad, nbloks * BLOCKLEN - N))
end
...
function Base.get(
        cards::Cards, keys::Union{AbstractArray, Tuple},
...
function Base.get(data::NamedTuple, key::AbstractString, default = nothing)
    return get(data, Symbol(key), nothing)
end
