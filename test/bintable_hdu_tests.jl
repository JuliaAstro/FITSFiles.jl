@testset "Bintable HDU" begin
    ...
    @test (
        length(data[1]) == 5 && size(hdu.data) == (3,) &&
            all([hdu.data[j][:par4] for j in 1:3] .== [1.0, 2.0, 3.0])
    )
    ...
    @test (
        length(hdu.data) == 5 && length(hdu.data[:par1]) == 3 &&
    ...
    @test (
        length(hdu.data[1]) == 5 && length(hdu.data) == 3 &&
            all([hdu.data[j][:par4] for j in 1:3] .== [1.0, 2.0, 3.0])
    )
